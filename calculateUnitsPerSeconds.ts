...
async function main() {
  ...
  if (!args['price']) {
    if (args['asset']) {
      ...
      console.error('You need to provide either an asset name with <--a> or a fixed price with <--p>');
    }
  ...
  if (tokenData.success) {
    ...
    const unitsPerSecond = (targetPrice * BigInt(10 ** 12) * BigInt(decimalsFactor)) / (xcmTotalCost * tokenPrice);
